...
//delete sql 의 조건절 컬럼이 여러개 일때는 dto가 될 수 있다. map도 종종 사용

//insert,update,delete 는 정수 리턴값으로 반영된 행의 개수를 전달 

//selectOne : sql 파라미터에 전달할 데이터를 메소드 인자로 함
//selectAll : 파라미터가 없으며 여러개의 행을 저장할 객체는 List 타입
...
public class StudentDao {
	...
	//select 모두 조회 - 조회 결과값을 List 객체로 리턴합니다. 메소드에서 조회된 결과만을 리턴
	//		출력 등 기타 기능들은 이 메소드를 사용하는 프로그램에서 구현합니다.
