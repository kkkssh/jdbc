...
// 이 소스에서 제일 먼저 해야할 것 - '데이터베이스 연결' 
public class OracleConnectionTest {

	public static void main(String[] args) {
		//0. Connection 은 인터페이스로 직접 객체를 생성하지 않고
		//	구현클래스가 있어야 한다. db 에서는 db드라이버가 접속하려는 db의 종류에 따라
		//	알아서(프록시) 구현클래스와 구현 객체를 만든다.
		Connection conn = null;
		
		
		//1. 아래 4개의 필수 연결 정보를 설정한다.
		//접속하고자 하는 서버의 주소 (포털 접속 https://www.naver.com 과 비슷한 개념)
		String url = "jdbc:oracle:thin:@localhost:1521:xe";
		
		//oracle.jdbc.driver는 ojdbc6.jar 에 포함된 패키지 이름
		...
		//		ㄴ연결 객체를 만들어주는 역할을 한다.
			Class.forName(driver);
			
		//3. DriverManager 클래스는 연결객체를 만든다. - 2번의 객체를 동작시킨다. 
		//이때 2번에서 만든 객체 즉 dbms 에 따라 구현객체가 만들어진다.
...
//	   : 서로 다른 소프트웨어 시스템 간의 연결을 위한 방식(라이브러리로 제공된다). 인터페이스는 소통
